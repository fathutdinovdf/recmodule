/* Инбокс «Мои задачи».
   ...
   намеренно: Заказчик обязан открыть карточку и прочитать обоснование. Значит
   уложиться в норматив 4 / 8 / 24 рабочих часа можно только одним способом —
   ...
   controlDelta) уже посчитан рабочими часами Заказчика, и пересчитывать его
...
const sut = (n) => `${n} ${plural(n, 'сутки', 'суток', 'суток')}`;
...
/* Окно эффекта, закрывающееся в ближайшие семь суток. Тот же отбор, что у
   глубокой ссылки index.html?alert=window, — специально один в один, чтобы
   счётчик инбокса и реестр не разошлись. */
  ...
  if (b.html) {
    return `<section class="blk blk--${b.tone}" id="${b.id}">
      <div class="blk__h">
        ...
        ${b.deep ? `<a class="blk__deep" href="${b.deep}">в реестре →</a>` : ''}
      </div>
      <div class="blk__why">${b.why}</div>
      ${b.html}
    </section>`;
  }
...
function buildExpert() {
  ...
  const blocks = [
    ...
    },
    {
      ...
      why: `Передача идёт только в рабочее окно Заказчика — пн–пт 09:00–24:00 по Когалыму,
            и норматив ответа стартует с момента передачи, а не регистрации. Действия
    ...
    },
    {
      ...
      title: 'Окна эффекта, закрывающиеся на этой неделе',
      why: `Окно идёт 90 суток от даты фактической реализации. Ближайшее из ваших
            ${windows.length} открытых закрывается ${windows.length ? fmt(windows[0].windowCloseAt, false) : '—'},
            то есть на этой неделе разбирать нечего.`,
      deep: 'index.html?alert=window',
      when: (r) => `до закрытия ${sut(daysTo(r.windowCloseAt))}`,
      empty: 'На этой неделе ни одно окно не закрывается.',
    },
  ];
...
function buildExpertLead() {
  ...
  /* «Согласовано, а режим не меняется» — единственное место, где у Исполнителя
     ...
     ничем не подсвечивается, и рекомендация может стоять месяцами. Порог в 21
     сутки взят как рабочая гипотеза — вопрос 3.7 открыт. */
  const stuck = approved.filter((r) => daysSince(r.repliedAt) > 21).sort(byOldestAnswer);
  ...
  const blocks = [
    ...
    },
    {
      ...
      when: (r) => `возражение ${fmt(r.dispute.at)}`,
      showExecutor: true,
      extra: (r) => `${r.executor} · наша дата ${fmt(r.factDate, false)} · предлагают ${fmt(r.dispute.proposedDate, false)}`,
      empty: 'Возражений нет.',
    },
    {
      id: 'b-stuck', tone: 'warn', act: true, rows: stuck,
      title: 'Согласовано больше 21 суток назад, реализации нет',
      ...
      when: (r) => `согласовано ${fmt(r.repliedAt, false)}`,
      showExecutor: true,
      extra: (r) => `${r.executor} · ждём смены режима ${sut(daysSince(r.repliedAt))}`,
    },
  ];
  ...
  const tiles = [
    ...
    { n: stuck.length, l: 'Из них дольше 21 суток', href: '#b-stuck' },
...
function buildEngineer() {
  ...
  const approved = my.filter((r) => r.status === 'approved').sort(byOldestAnswer);
  const windows = my.filter((r) => r.status === 'windowOpen')
    .sort((a, b) => toDate(b.factFixedAt) - toDate(a.factFixedAt));
  ...
  const blocks = [
    ...
    },
    {
      ...
      why: `Норматив ещё не истёк. Остаток считается рабочими часами Заказчика,
            пн–пт 09:00–24:00: календарные сутки и сутки норматива — разные величины.`,
      when: (r) => `передано ${fmt(r.sentAt)}`,
      showExecutor: true,
  ...
  const blocks = [
    {
      ...
      title: 'Самые давние просрочки',
      why: `Ответа нет дольше всего именно по этим рекомендациям. Норматив по договору —
            4 / 8 / 24 рабочих часа с момента передачи; приоритет I просрочен
            в ${overdueI.length} случаях.`,
      deep: 'index.html?alert=overdue',
      when: (r) => `передано ${fmt(r.sentAt)}`,
      showExecutor: true,
      extra: (r) => `${r.field} · срок был ${fmt(r.dueAt)}`,
    },
    {
      id: 'b-fields', tone: 'calm', count: all.length,
      title: 'Где горит по объектам',
      why: `Свод по узлам дерева ВМАП. Месторождение в дереве существует парой
            «месторождение × цех», поэтому Южно-Ягунское стоит четырьмя строками, —
            свести их в одну можно будет, когда Заказчик определится с вопросом 1.1.`,
      html: sumTable(
...
/* ------------------------------ инбокс администратора ------------------------------ */

function buildAdmin() {
  const all = DATA;
  /* Записи, где объект рекомендации пришлось назначить руками. В боевом реестре
     такие пишут на АГЗУ, а объектом рекомендации может быть только скважина —
     запись привязывается к скважине, на замер которой АГЗУ влияет. */
  const remapped = all.filter((r) => r.note);
  const wells = new Set(all.map((r) => r.well)).size;
  const fields = new Set(all.map((r) => r.field)).size;

  const blocks = [
    {
      id: 'b-objects', tone: 'hot', act: true, rows: remapped,
      title: 'Объект рекомендации назначен вручную',
      why: `Объектом может быть только скважина, но в боевом реестре встречаются записи,
            выписанные на АГЗУ. Такие привязаны к скважине, на замер которой это влияет,
            и требуют сверки — правило разрешать куст как объект пока не принято.`,
      when: (r) => `зарегистрирована ${fmt(r.regDate)}`,
      showExecutor: true,
      extra: (r) => r.note,
    },
    {
      id: 'b-service', tone: 'calm',
      title: 'Служебная сводка',
      why: `Стартовая страница администратора — «Пользователи и роли», а не инбокс:
            задач по рекомендациям у него нет. Здесь остаётся только состояние модуля.`,
      html: `<div class="kv">
        ${[['Рекомендаций в реестре', all.length],
          ['Объектов дерева ВМАП', fields],
          ['Скважин с рекомендациями', wells],
          ['Экспертов Исполнителя', EXECUTORS.length],
          ['Ответственных Заказчика', CUSTOMERS.length],
          ['Статусов в модели', STATUSES.length]]
          .map(([k, v]) => `<div class="kv__i"><span class="kv__k">${k}</span><span class="kv__v">${v}</span></div>`).join('')}
      </div>
      <div class="blk__empty">Синхронизация справочников из ois_vmap, консюмер Kafka, суточный
        снимок телеметрии и журнал ошибок в данных макета не смоделированы — это следующий
        заход, вместе с окном «Пользователи и роли».</div>`,
    },
  ];

  const tiles = [
    { n: all.length, l: 'Рекомендаций в реестре', href: 'index.html' },
    { n: fields, l: 'Объектов дерева ВМАП', href: '#b-service' },
    { n: wells, l: 'Скважин', href: '#b-service' },
    { n: remapped.length, l: 'Объект назначен вручную', href: '#b-objects' },
  ];
...
function renderNow() {
  ...
  $('#nowline').innerHTML = `<svg class="ic16"><use href="#i-clock"/></svg>
    ${fmt(NOW)} · рабочее окно Заказчика ${before
