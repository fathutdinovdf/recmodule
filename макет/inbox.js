  ...
  const blocks = [
    {
      ...
      /* Норматив 4 / 8 / 24 назван рабочим, а не договорным, сознательно: чисел
         этих в договоре нет, они пришли из Формы 2 (решение 64). Руководителю
         Заказчика этот текст читать первым — здесь ошибка в ссылке на договор
         стоила бы дороже всего. */
      why: `Ответа нет дольше всего именно по этим рекомендациям. Рабочий норматив —
            4 / 8 / 24 рабочих часа с момента передачи по приоритетам I / II / III;
            приоритет I просрочен в ${overdueI.length} случаях.`,
      deep: 'index.html?alert=overdue',
      when: (r) => `передано ${fmt(r.sentAt)}`,
      showExecutor: true,
      extra: (r) => `срок был ${fmt(r.dueAt)} · ${r.status === 'review'
...
function buildAdmin() {
    ...
    },
    {
      ...
      html: `<div class="kv">
        ${[['Рекомендаций в реестре', all.length],
          /* Не «сколько узлов в дереве», а «сколько задействовано»: справочник
             ВМАП шире реестра, и путать эти два числа администратору нельзя. */
          ['Узлов дерева ВМАП с рекомендациями', `${fields} из ${FIELDS.length}`],
  ...
  const tiles = [
    ...
    { n: fields, l: 'Узлов дерева ВМАП', href: '#b-service' },
