...
/* Роли и зоны лежат в data.js: по той же зоне отбирает реестр, и держать её
   в двух местах значило бы показывать одному человеку разное число задач на
   двух экранах. Здесь к пользователю добавляется только сборка его инбокса.
   ...
   Администратора в «Моих задачах» нет (решение 82): задач по рекомендациям у
   него не бывает, его стартовая страница — «Пользователи и роли». */
const BUILDERS = {
  expert: buildExpert,
  expertLead: buildExpertLead,
  engineer: buildEngineer,
  customerLead: buildCustomerLead,
};

const ROLES = USERS.filter((u) => u.inInbox !== false).map((u) => ({
  ...u,
  mine: (r) => inScope(u, r),
  build: BUILDERS[u.key],
}));
