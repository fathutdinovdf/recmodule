...
/* От чьего имени открыта карточка. Тем же параметром, что в инбоксе и реестре,
   чтобы переход между экранами не терял роль.

   По умолчанию — руководитель Заказчика, а не эксперт Исполнителя: карточку
   открывают, чтобы посмотреть весь её функционал, а решение по рекомендации
   принимает Заказчик. С ролью Исполнителя кнопок решения не было бы вовсе,
   и демонстрация упиралась бы в необходимость дописать роль в адрес. */
const USER = USERS.find((u) => u.key === params.get('role'))
  || USERS.find((u) => u.canDecide && !u.zone.length)
  || USERS[0];
...
/* REJECT_REASONS и CLARIFY_REASONS переехали в data.js: справочник нужен и
   форме решения, и генератору тестовых данных. Пока список жил здесь, генератор
   про него не знал и отправлял записи в «Отклонено» вообще без обоснования. */
...
function decisionBlock() {
  if (rec.decision) {
    ...
    return `<div class="decision decision--done">
      ...
      ${rec.rejectReason ? `<div class="block__h" style="margin-top:12px">${
