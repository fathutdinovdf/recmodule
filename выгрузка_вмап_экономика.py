...
скважины. Для расчёта эффекта этого мало, и вот почему.
  ...
  2. Способ эксплуатации. У фонтанной скважины обнуляются сразу две статьи
     ...
     стоимость обслуживания. Поле Wells.OperationMode в базе оказалось почти
     пустым: заполнено у пяти скважин из семи тысяч. Настоящий способ
     эксплуатации лежит параметром 70 в WellData.
...
def main():
    ...
    wells, ош = выбрать(cur, f"""
        SELECT "Id", "Code", "Name", "OrganizationUnitId"
    ...
    # Значения способа эксплуатации: их надо увидеть глазами, потому что
    # параметр 70 текстовый, а 205 номерной, и что в них лежит — вопрос.
    режимы = defaultdict(int)
    for w in наши:
        v = по_скважинам.get(w["Id"], {}).get(70)
        if v is not None:
            режимы[str(v)] += 1
    итог["operation_modes"] = dict(режимы)
    if режимы:
        print("\nСпособ эксплуатации:", ", ".join(f"{k} — {v}" for k, v in
                                                  sorted(режимы.items(), key=lambda x: -x[1])))
