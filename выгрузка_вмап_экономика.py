...
скважины. Для расчёта эффекта этого мало, и вот почему.
  ...
  2. Способ эксплуатации. У фонтанной скважины обнуляются сразу две статьи
     затрат — услуги ЭПУ и электроэнергия на подъём, — а у ЭЦН и ШГН разная
     стоимость обслуживания. Wells.OperationMode заполнен у всех скважин, но
     это число: по поддереву Когалымнефтегаза 4006 скважин со значением 0,
     четыре с 1 и одна с 2. Что значит каждый код, из самой таблицы не видно,
     поэтому скрипт берёт его вместе с текстовым параметром 70 из WellData и
     сводит одно с другим — расшифровка получается из данных, а не из
     догадки.
...
ПАРАМЕТРЫ = {
    70:  "способ эксплуатации",
    205: "способ эксплуатации номерной",
...
def main():
    ...
    # ------------------------------------------------------------------
    # 3. Расшифровка кода способа эксплуатации
    # ------------------------------------------------------------------
    # Wells.OperationMode — число, и что означает каждый код, из таблицы не
    # видно. Сводим его с текстовым параметром 70 из WellData: если у скважин
    # с кодом 0 в параметре стоит «ЭЦН», расшифровка получена из данных.
    # Ноль здесь — полноценное значение, а не пустота; на этом уже один раз
    # споткнулись, посчитав его отсутствием.
    свод = defaultdict(lambda: defaultdict(int))
    for w in наши:
        код = w["OperationMode"]
        текст = по_скважинам.get(w["Id"], {}).get(70)
        свод["null" if код is None else str(код)]["—" if текст is None else str(текст)] += 1

    итог["operation_mode_decode"] = {к: dict(v) for к, v in свод.items()}
    print("\nWells.OperationMode × параметр 70 «Способ эксплуатации»:")
    for код in sorted(свод, key=lambda k: -sum(свод[k].values())):
        расшифровки = ", ".join(f"{т} — {n}" for т, n in
                                sorted(свод[код].items(), key=lambda x: -x[1]))
        print(f"   код {код:>4}: {расшифровки}")

    # Номерной параметр 205 — третий источник того же самого. Если он
    # заполнен, по нему видно, совпадает ли нумерация с Wells.OperationMode.
    номерной = defaultdict(int)
    for w in наши:
        v = по_скважинам.get(w["Id"], {}).get(205)
        if v is not None:
            номерной[f'{w["OperationMode"]} → {v}'] += 1
    if номерной:
        итог["operation_mode_numeric"] = dict(номерной)
        print("   параметр 205 против Wells.OperationMode:",
              ", ".join(f"{k}: {n}" for k, n in sorted(номерной.items(), key=lambda x: -x[1])[:8]))
