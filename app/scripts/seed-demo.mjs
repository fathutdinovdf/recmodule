  ...
  for (const [статус, сколько] of РАСКЛАД) {
    for (let i = 0; i < сколько; i++) {
      ...
      const { rows } = await client.query(`
        ...
        RETURNING id
      `, [num, статус, направление, черновик ? null : приоритет,
      ...
      if (!черновик) {
        ...
        `, [recId, +(5 + rnd() * 40).toFixed(3), +(2 + rnd() * 18).toFixed(3), int(300, 900),
          ...
          'База по семи суткам до мероприятия, замеры АГЗУ.']);
      }
      ...
      if (статус === 'review' && sentAt) {
        await событие(плюсЧасов(sentAt, 1), 'status', решающий, имяРешающего, 'sent', 'review', 'Взято на рассмотрение');
      }
      ...
      if (статус === 'windowOpen' || статус === 'windowClosed') {
        ...
        `, [recId, день(factDate), плюсЧасов(factDate, int(6, 40)), автор, имяАвтора,
          полнота === 'partial'
            ? 'Режим изменён частично: частота выведена не до рекомендованной.' : null,
          день(openAt), день(closeAt), closedAt, закрытоДосрочно]);

        await client.query('UPDATE rec.recommendations SET completeness = $2 WHERE id = $1',
          [recId, полнота]);
