 ...
 *
 * Правило пересчёта — РЕШЕНИЕ ПОЛЬЗОВАТЕЛЯ, важное для будущей архитектуры:
 * страница ничего не пересчитывает. Она читает готовое из `rec.effect_daily` и
 * только. Пересчёт — дело бэкенда, который в рабочем контуре поднят постоянно;
 * запросы к чужому стенду не должны зависеть от того, открыл ли кто-то вкладку.
 * Раньше открытое окно считалось заново при КАЖДОМ открытии вкладки: тот же
 * поход на стенд ВМАП повторялся столько раз, сколько раз на карточку
 * посмотрели, хотя замеры приезжают раз в сутки.
 *
 *   окно открыто  — отдаём кэш; свежесть обеспечивает пересчёт на бэкенде;
 *   окно закрыто  — отдаём кэш и на стенд не ходим вовсе: закрытие фиксирует
 *                   цифру.
 *
 * Единственное исключение — холодный старт: кэша нет вообще ни одной строки.
 * Тогда считаем один раз прямо здесь, иначе на демо-стенде и в первый день
 * работы вкладка была бы пустой у всех сразу. Когда появится постоянный
 * бэкенд с расписанием, исключение убирается вместе с вызовом `пересчитать`
 * из `getEffect` — сам пересчёт уже вынесен в отдельную функцию именно ради
 * этого.
 */
...
export async function getEffect(card: Card): Promise<EffectView | null> {
  ...
  /* Кэш читается всегда, а не только у закрытого окна: страница обязана
     обходиться сохранённым. */
  const кэш = await readCache(card.id);
  ...
  /* Холодный старт: сохранённого расчёта нет ни одной строки. Считаем один
     раз прямо здесь — иначе вкладка была бы пуста до первого прохода
     бэкенда. Это единственное место, где страница ходит на стенд. */
  const расчёт = await вычислить(card, открыто, конец);
...
/* Один вызов расчёта на два входа: холодный старт страницы и пересчёт с
   бэкенда. Собран отдельно, чтобы аргументы окна не разъехались между ними. */
function вычислить(card: Card, windowFrom: Date, windowTo: Date) {
...
/**
 * Пересчёт окна: поход на стенд ВМАП и перезапись кэша.
 *
 * Точка входа для фонового расчёта, а не для рендера страницы. Сейчас её
 * зовёт маршрут `POST /api/effect/recalc`, дальше позовёт постоянный бэкенд по
 * расписанию. Вынесена отдельно именно для того, чтобы у страницы не было
 * соблазна дёрнуть пересчёт «на всякий случай».
 *
 * Возвращает число пересчитанных суток или null, если окна ещё нет.
 */
export async function recalcEffect(card: Card): Promise<number | null> {
  const impl = card.implementation;
  if (!impl || card.wellId === null || card.fieldId === null) return null;
  ...
  const расчёт = await вычислить(card, открыто, конец);
  await writeCache(card.id, card.baseline?.id ?? null, расчёт.days, new Date());
  return расчёт.days.length;
}
