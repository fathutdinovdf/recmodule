 ...
 *
 * Экспортируется для инбокса (db/inbox.ts): его блоки обязаны видеть те же
 * строки и тот же control_kind, что и реестр, — второй CTE разошёлся бы с
 * этим при первой же правке.
 */
export async function основа(): Promise<string> {
