...
/**
 * Соседи по реестру для листалки в шапке — порядок по умолчанию, новые сверху.
 *
 * Это только запасной вариант на случай, если карточку открыли не из
 * отфильтрованной таблицы (прямая ссылка, черновик и т. п.): считается один
 * раз серверно вместе с остальной шапкой layout.tsx и передаётся в `Pager`
 * (`./pager.tsx`) как fallback. Если карточку открыли из реестра с активным
 * отбором (`?from=…`), `Pager` на клиенте подменяет результат на
 * `getFilteredNeighbours` из `db/recommendations.ts` — тот же порядок и те же
 * условия, что были в таблице.
 */
