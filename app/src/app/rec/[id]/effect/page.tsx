    ...
    <div className="eff">
      ...
      <section>
        ...
        <div className="eff-legend" style={{ marginTop: 'var(--item-gap-vertical-s)' }}>
          ...
          <span>{РУЧНОЙ_ИСТОЧНИК
            ? 'разрыв линии — сутки без внесённого дебита, они вне расчёта'
            : 'разрыв линии — суток без замеров и без чего протянуть'}</span>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="eff-quality">
          ...
          {/* Обе плитки — про плотность телеметрии: сколько суток опёрлись на
              протянутое значение и насколько. При ручном вводе протянутого
              дебита не бывает вовсе (сутки либо внесены, либо пусты), обе
              показывали бы 0 и 100 % — цифры, которые ничего не измеряют, но
              читаются как оценка качества. */}
          {!РУЧНОЙ_ИСТОЧНИК && (
            <ЯчейкаКачества v={String(eff.daysCarried)}
              k="суток без своих замеров, значение протянуто" />
          )}
        ...
        <div className="eff__note" style={{ marginTop: 'var(--item-gap-vertical-s)' }}>
          {РУЧНОЙ_ИСТОЧНИК ? (
            <>
              Дебит и обводнённость внесены вручную во вкладке «Суточные данные»:
              стенд ВМАП на этом контуре недоступен. Сутки без внесённого дебита
              выпадают из расчёта целиком — со вчерашнего дня значение не тянется.
              Обводнённость, наоборот, протягивается с последних известных суток:
              её определяют лабораторно и много реже дебита.
              {' '}Ноль в дебите означает остановленную скважину и в расчёт идёт,
              давая отрицательный прирост.
            </>
          ) : (
            <>
