...
import { getWell, РУЧНОЙ_ИСТОЧНИК } from '@/db/wells-data';
    ...
    <div className="eff">
      ...
      <section>
        ...
        <div className="eff-quality">
          ...
          {/* Протяжка есть в обоих режимах, поэтому плитки одни и те же.
              Меняется только слово: со стенда значение приходит замером, при
              ручном режиме — внесённым суточным значением. */}
          <ЯчейкаКачества v={String(eff.daysCarried)}
            k={РУЧНОЙ_ИСТОЧНИК
              ? 'суток без своего значения, взято протянутое'
              : 'суток без своих замеров, значение протянуто'} />
          <ЯчейкаКачества v={String(eff.daysTotal - eff.daysWithData)} k="суток без данных вовсе" />
          <ЯчейкаКачества
            v={`${число(среднееПокрытие(eff.days) * 100, 0)} %`}
            k={РУЧНОЙ_ИСТОЧНИК
              ? 'доля суток с собственным внесённым значением'
              : 'средняя опора суток на собственные замеры'} />
        </div>
        <div className="eff__note" style={{ marginTop: 'var(--item-gap-vertical-s)' }}>
          {РУЧНОЙ_ИСТОЧНИК ? (
            <>
              Значения внесены вручную во вкладке «Суточные данные»: стенд ВМАП
              на этом контуре недоступен. Сутки без своего значения считаются по
              последнему известному — оно протягивается вперёд, пока не появится
              новое, так же как это делает телеметрия. В базу такие сутки не
              берутся: договор требует кондиционных, то есть собственных
              значений.
              {' '}Ноль означает остановленную скважину и в расчёт идёт, давая
              отрицательный прирост. Пустое поле — не ноль: это «значения нет».
              {' '}Протяжка не отличает остановленную скважину от той, по которой
              просто не внесли данные, — за этим следит доля суток с собственным
              значением выше.
            </>
          ) : (
            <>
  ...
  return (
    <div className="eff">
      ...
      {/* База показывается и до открытия окна — тем же блоком, что и после.
          Причём ВСЕГДА, а не только когда она уже внесена: именно здесь её и
          вносят, и до фиксации реализации это единственное место, где база
          вообще существует на экране. Раньше блок при отсутствии базы
          скрывался, и внести её после регистрации было негде. */}
      {!мёртвая && (
        <БлокБазы card={card} user={user}
                  заголовок={card.baseline ? 'База уже внесена' : 'Базовые значения'}
                  форма={форма} />
      )}
    </div>
  );
