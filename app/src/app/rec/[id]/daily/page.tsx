...
export default async function Страница({ params }: { params: Promise<{ id: string }> }) {
  ...
  /* Откуда возьмётся протянутое значение на сутки без собственного. Запас
     ...
     разошлись бы в том, какие сутки считаются протянутыми.

     Нужен не флаг, а ИСТОЧНИК: последнее внесённое значение может лежать до
     начала показываемого отрезка, и тогда клетка помечена протяжкой, а
     объяснения этому на экране нет. Дату и величину показывает окно дня. */
  const доНачала = await factsInRange(
    card.wellId, new Date(начало.getTime() - 30 * СУТКИ), new Date(начало.getTime() - СУТКИ),
  );
  let последнее: { iso: string; value: number } | null = null;
  for (const ф of [...доНачала.values()].sort((a, b) => a.date.getTime() - b.date.getTime())) {
    if (ф.qzh !== null) последнее = { iso: iso(ф.date), value: ф.qzh };
  }
  ...
  for (let t = начало.getTime(); t <= конец.getTime(); t += СУТКИ) {
    ...
    const ф = факт.get(k);
    /* Источник читается ДО обновления: протянуть можно только то, что внесли
       раньше этих суток, а не в них самих. */
    const источник = последнее;
    if (ф?.qzh !== null && ф?.qzh !== undefined) последнее = { iso: iso(д), value: ф.qzh };
    дни.push({
      iso: iso(д),
      qzh: ф?.qzh ?? null,
