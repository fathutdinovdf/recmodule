...
export function КалендарьСуток({
  ...
  const считаемые = дни.filter((d) => d.вОкне && !d.будущее);
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">
            Отмечены сутки окна эффекта — те, по которым считаются деньги.
            Сутки без своего значения берут последнее известное: оно
            протягивается вперёд, как в телеметрии.
          </p>
        </div>
...
function КлеткаДня({ день, сегодня, recId, можноПравить, выключен, className: _чужие, ...props }: {
  ...
  const вРасчёте = !!день && день.вОкне;
