 ...
 *
 * Раскрашиваются только сутки расчёта этой рекомендации — база и окно.
 * Соседние остаются приглушёнными: их состояние к карточке не относится.
 */
...
function КлеткаДня({ день, сегодня, recId, можноПравить, выключен, className: _чужие, ...props }: {
  ...
  /* Раскрашиваются только сутки, которые участвуют в расчёте ЭТОЙ
     рекомендации. Остальные — соседние по календарю — остаются приглушёнными:
     их состояние к этой карточке отношения не имеет, и метки на них
     превращали сетку в пестроту, не отвечая ни на один вопрос.
  ...
  const вРасчёте = !!день && (день.вОкне || день.вБазе);
