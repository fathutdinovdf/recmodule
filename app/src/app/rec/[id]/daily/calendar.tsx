 ...
 *
 * Заливка клетки кодирует полноту суток, а не «выбранность»: сплошная — есть
 * оба значения, кольцо — только одно, пунктир — сутки внутри расчёта пустые.
 * Точка в углу значит, что значение уже правили, и в окне есть история.
 */
...
export function КалендарьСуток({
  ...
  const полных = считаемые.filter((d) => d.qzh !== null && d.watercut !== null).length;
  ...
  const модификаторы = React.useMemo(() => ({
    полные: дни.filter((d) => d.qzh !== null && d.watercut !== null).map((d) => дата(d.iso)),
    частичные: дни.filter((d) => (d.qzh === null) !== (d.watercut === null)).map((d) => дата(d.iso)),
    пробел: считаемые.filter((d) => d.qzh === null && d.watercut === null).map((d) => дата(d.iso)),
    ...
    <div className="flex flex-col gap-4 p-[var(--section-padding)]">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">
            Дебит жидкости и обводнённость за сутки. Расчёт эффекта идёт по
            фактическим суткам, поэтому незаполненный день не «усредняется», а
            выпадает из итога.
          </p>
        </div>
      ...
      {пробелов > 0 && !окноЗакрыто && (
        <div className="flex items-start gap-3 rounded-md border border-[var(--border-divider)]
          ...
          <TriangleAlert className="mt-0.5 size-4 shrink-0 text-[var(--status-warning)]" aria-hidden />
          <p className="text-[var(--text-secondary)]">
            Не заполнено суток: <b>{пробелов}</b>. Каждые такие сутки уменьшают
            накопленный эффект — они входят в срок окна, но не в расчёт.
          </p>
        </div>
      )}
...
function Легенда() {
  const точки = [
    ['bg-[var(--accent-primary)]', 'заполнено полностью'],
    ['ring-2 ring-inset ring-[var(--accent-primary)]', 'заполнено наполовину'],
    ['border border-dashed border-[var(--status-warning)]', 'пробел в расчёте'],
...
function КлеткаДня({ день, сегодня, recId, можноПравить, выключен, className, ...props }: {
  ...
  const частично = !!день && (день.qzh === null) !== (день.watercut === null);
  const пробел = !!день && !день.будущее && (день.вОкне || день.вБазе)
    && день.qzh === null && день.watercut === null;
  ...
  const вид = [
    ...
    частично ? 'ring-2 ring-inset ring-[var(--accent-primary)]' : '',
    пробел ? 'border border-dashed border-[var(--status-warning)]' : '',
    !полон && !частично && !пробел ? 'hover:bg-accent' : '',
    ...
    <>
      ...
      <form action={отправить} className="px-4 py-3">
        ...
        <p className="mt-2 text-xs text-[var(--text-tertiary)]">
          Пустое поле — «нет данных». Ноль значит остановленную скважину.
        </p>
