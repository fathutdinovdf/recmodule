...
export function Чат({ recId, начальные, люди, я, обсуждаемо, причина }: {
  ...
  async function отправить(ч: Черновик) {
    ...
    try {
      ...
      /* Подмена, а не «убрать и добавить»: строка остаётся на месте, меняется
         только её содержимое — иначе реплика моргнула бы. Но пока ждали ответ,
         тот же комментарий мог уже прийти по SSE (сервер рассылает его сразу
         после вставки) и лечь отдельной записью с тем же реальным ключом —
         такую надо убрать, иначе получится дубль с неуникальным key. */
      setЗаписи((было) => было
        .filter((x) => x.key !== запись.key)
        .map((x) => x.key === временный ? запись : x));
    } catch (e) {
  ...
  /* Сутки — единственная группировка ленты: время внутри дня отвечает «когда»,
     ...
     Группируем по Map, а не по «совпадает с последней группой»: слияние
     свежих записей с сервера (см. эффект выше) вставляет их в середину
     массива, а не в хронологическом порядке, и соседство одного дня не
     гарантировано — при разрыве получались бы два <section> с одним key. */
  const дниMap = new Map<string, { key: string; label: string; items: FeedItem[] }>();
  for (const e of записи) {
    let группа = дниMap.get(e.dayKey);
    if (!группа) {
      группа = { key: e.dayKey, label: e.dayLabel, items: [] };
      дниMap.set(e.dayKey, группа);
    }
    группа.items.push(e);
  }
  const дни = [...дниMap.values()];
