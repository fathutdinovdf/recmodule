...
export function ОкноФиксации({ card, нетБазы, стартОткрыто }: {
  card: Card; нетБазы: boolean; стартОткрыто: boolean;
}) {
  ...
  return (
    <ActionDialog
      ...
      /* Кнопка гаснет, пока нет базы: фиксация открывает окно эффекта, а
         договор требует определить базу ДО реализации. Серверная проверка в
         actions.ts остаётся независимо — выключенная кнопка подсказка, а не
         защита. */
      trigger={<Button disabled={нетБазы}>Зафиксировать реализацию</Button>}
    >
